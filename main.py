...
def relatorio_contatos_responsaveis():
    ...
    if _cont and hasattr(_cont, 'gerar_pdf_contatos'):
        try:
            ...
            try:
                _fn = cast(Any, _cont).gerar_pdf_contatos
                # Envolvemos o chamador para garantir que o `ano_param` seja passado corretamente
                _run_report_module_returning_buffer(lambda: _fn(ano_param), "Contatos")
            except Exception as e:
                messagebox.showerror("Erro", f"Falha ao agendar relatório de contatos: {e}")
        except Exception as e:
            messagebox.showerror("Erro", f"Falha ao preparar relatório de contatos: {e}")
...
def obter_ano_letivo_atual() -> int:
    ...
    """
    try:
        # Usar o context manager com get_cursor() (retorna cursores com dictionary=True)
        with get_cursor() as cur:
            ...
            # Normalizar resultado: pode ser tuple/list ou dict (dictionary cursor)
            if res is None:
                return 1
            if isinstance(res, dict):
                # preferir chave 'id', senão pegar primeiro valor
                id_val = res.get('id') if 'id' in res else (list(res.values())[0] if res else None)
            else:
                id_val = res[0] if len(res) > 0 else None

            return converter_para_int_seguro(id_val) if id_val is not None else 1
    except Exception as e:
...
def verificar_matricula_ativa(aluno_id):
    ...
    try:
        with get_cursor() as cursor:
            ...
            # Normalizar resultado (dict ou tuple)
            if isinstance(resultado_ano, dict):
                ano_letivo_id = resultado_ano.get('id') if 'id' in resultado_ano else (list(resultado_ano.values())[0] if resultado_ano else None)
            else:
                ano_letivo_id = resultado_ano[0] if len(resultado_ano) > 0 else None
...
def verificar_historico_matriculas(aluno_id):
    ...
    try:
        with get_cursor() as cursor:
            ...
            # Normalizar rows que podem ser dicts (dictionary cursor) para listas/tuples
            if resultados and isinstance(resultados[0], dict):
                resultados = [tuple(r.values()) for r in resultados]
            ...
            if not resultados:
                ...
                if isinstance(ano_2024, dict):
                    vals = list(ano_2024.values())
                    ano_2024 = tuple(vals)
                if ano_2024:
                    ...
                    if isinstance(resultado_count, dict):
                        # extrair primeiro valor
                        resultado_count = tuple(resultado_count.values())
            ...
            if resultados:
                ...
                if todos_anos and isinstance(todos_anos[0], dict):
                    todos_anos = [tuple(r.values()) for r in todos_anos]
    ...
    try:
        ...
        def salvar_matricula():
            ...
            try:
                ...
                # rollback handled by get_cursor on exception
            ...
            if aluno_id:
                try:
                    with get_cursor() as cursor:
                        ...
                        responsaveis = [row[0] for row in resultados if row and row[0]]
...
def atualizar_tabela_principal(forcar_atualizacao=False):
    ...
    try:
        ...
        # Conectar ao banco de dados usando helper de cursor (retorna cursores com dict-friendly rows)
        with get_cursor() as cursor:
        ...
        try:
            for resultado in resultados:
                # Normalizar linha: aceitar tanto dict-rows quanto tuplas/listas
                if isinstance(resultado, dict):
                    row = list(resultado.values())
                else:
                    row = list(resultado)
...
def selecionar_ano_para_boletim(aluno_id):
    ...
    Args:
        aluno_id: ID do aluno
    """
    # Obter informações do aluno e anos letivos
    try:
        # Nome do aluno (conexão curta)
        with get_cursor() as cursor:
            ...
            if not resultado_nome:
                messagebox.showerror("Erro", "Aluno não encontrado.")
                return
            if isinstance(resultado_nome, dict):
                nome_aluno = list(resultado_nome.values())[0]
            else:
                nome_aluno = resultado_nome[0]

        # Obter anos letivos nos quais o aluno teve matrícula (helper já retorna a lista)
        ...
        # Criar dicionário para mapear anos letivos e status (usar cursor curto para as consultas)
        anos_info = {}
        try:
            with get_cursor() as cursor:
                for ano_info in anos_letivos:
                    ...
                    # cursor may return dict rows or tuples; normalize
                    if isinstance(status_result, dict):
                        status = list(status_result.values())[0] if status_result else "Desconhecido"
                    else:
            ...
            # Falha ao obter status de anos não deve impedir a exibição da UI; deixar anos_info possivelmente vazio
            logger.exception("Falha ao obter status de matrículas por ano para o aluno %s", aluno_id)
...
def editar_matricula(aluno_id):
    ...
    try:
        # Buscar informações iniciais em conexão curta usando helper de cursor
        with get_cursor() as cur_init:
            cur_init.execute("SELECT nome FROM alunos WHERE id = %s", (aluno_id,))
            resultado_nome = cur_init.fetchone()
            if not resultado_nome:
                messagebox.showerror("Erro", "Aluno não encontrado.")
                return
            # Normalizar fetchone (dict vs tuple)
            if isinstance(resultado_nome, dict):
                nome_aluno = list(resultado_nome.values())[0]
            else:
                nome_aluno = resultado_nome[0]
            ...
            if not resultado_ano:
                ...
                return
            if isinstance(resultado_ano, dict):
                ano_letivo_id, ano_letivo = list(resultado_ano.values())
            else:
                ano_letivo_id, ano_letivo = resultado_ano
            ...
            try:
                with get_cursor(commit=True) as cur:
                    ...
                    if historico_existente:
                        ...
                        if isinstance(historico_existente, dict):
                            existing_id = list(historico_existente.values())[0]
                        else:
                            existing_id = historico_existente[0]
                        cur.execute("""
                            UPDATE historico_matricula 
                            SET data_mudanca = %s
                            WHERE id = %s
                        """, (data_formatada, int(str(existing_id)) if existing_id is not None else 0))
                    else:
                        cur.execute("""
                            INSERT INTO historico_matricula (matricula_id, status_anterior, status_novo, data_mudanca)
                            VALUES (%s, %s, %s, %s)
