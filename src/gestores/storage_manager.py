"""Wrapper do gerenciador de armazenamento.

Mantém compatibilidade com importações existentes e delega a lógica
para `storage_manager_impl.py`.
"""

import storage_manager_impl as _impl
from src.gestores.storage_manager_impl import *  # re-exporta API pública

__all__ = getattr(_impl, "__all__", [name for name in dir(_impl) if not name.startswith("_")])

"""Wrapper mínimo: exporta symbols da implementação limpa.

Este arquivo permanece com o nome `storage_manager.py` para compatibilidade
com importações existentes, mas delega a lógica para
`storage_manager_impl.py`.
"""
from src.gestores.storage_manager_impl import *
"""
Gerenciador de armazenamento (modo único): local OU Google Drive.
Mantém a mesma estrutura de pastas no destino escolhido.
"""
import os
import shutil
...
class StorageManager:
    """Gerenciador de armazenamento que salva OU localmente OU no Google Drive.

    O modo é definido na inicialização. A API principal é:
      - salvar_arquivo(...)
      - listar_arquivos(...)
      - obter_info_armazenamento()
    """
    ...
    def __init__(self, base_local: Optional[str] = None, usar_google_drive: Optional[bool] = None):
        # Determina uso de Google Drive (auto-detect se None)
        ...
        # Configurar caminhos conforme modo
        ...
        # Criar estrutura no destino escolhido
        self._inicializar_estrutura()

        logger.info(f"StorageManager inicializado: modo={self.modo} caminho={self.base_storage}")
    ...
    def _detectar_caminho_drive(self) -> Optional[Path]:
        ...
        for caminho in possiveis:
            try:
                if caminho.exists():
                    logger.info(f"Google Drive montado encontrado em: {caminho}")
                    return caminho
            except Exception:
                continue
        logger.debug("Google Drive não montado localmente")
        return None

    def _inicializar_estrutura(self) -> None:
        try:
            self._criar_estrutura_recursiva(self.base_storage, self.ESTRUTURA_PASTAS)
            logger.info(f"Estrutura criada em: {self.base_storage}")
        except Exception as e:
            logger.exception(f"Erro ao criar estrutura: {e}")
            raise
    ...
    def obter_caminho_turma(self, nome_turma: str) -> Optional[Path]:
        ...
        logger.warning(f"Turma não mapeada: {nome_turma}")
        return None
