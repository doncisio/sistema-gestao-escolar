...
def obter_dados_alunos(cursor):
    ...
    SELECT
        ...
        COALESCE(SUM(CASE WHEN d.nome = 'LÍNGUA PORTUGUESA' AND d.nivel_id IN (2, 3) THEN COALESCE(n.nota, 0) END), 0) / NULLIF(COUNT(CASE WHEN d.nome = 'LÍNGUA PORTUGUESA' AND d.nivel_id IN (2, 3) THEN COALESCE(n.nota, 0) END), 0) AS 'NOTA_PORTUGUES',
        COALESCE(SUM(CASE WHEN d.nome = 'MATEMÁTICA' AND d.nivel_id IN (2, 3) THEN COALESCE(n.nota, 0) END), 0) / NULLIF(COUNT(CASE WHEN d.nome = 'MATEMÁTICA' AND d.nivel_id IN (2, 3) THEN COALESCE(n.nota, 0) END), 0) AS 'NOTA_MATEMATICA',
        COALESCE(SUM(CASE WHEN d.nome = 'HISTÓRIA' AND d.nivel_id IN (2, 3) THEN COALESCE(n.nota, 0) END), 0) / NULLIF(COUNT(CASE WHEN d.nome = 'HISTÓRIA' AND d.nivel_id IN (2, 3) THEN COALESCE(n.nota, 0) END), 0) AS 'NOTA_HISTORIA',
        ...
        COALESCE(SUM(CASE WHEN d.nome = 'CIÊNCIAS' AND d.nivel_id IN (2, 3) THEN COALESCE(n.nota, 0) END), 0) / NULLIF(COUNT(CASE WHEN d.nome = 'CIÊNCIAS' AND d.nivel_id IN (2, 3) THEN COALESCE(n.nota, 0) END), 0) AS 'NOTA_CIENCIAS',
        ...
        COALESCE(SUM(CASE WHEN d.nome = 'EDUCAÇÃO FÍSICA' AND d.nivel_id IN (2, 3) THEN COALESCE(n.nota, 0) END), 0) / NULLIF(COUNT(CASE WHEN d.nome = 'EDUCAÇÃO FÍSICA' AND d.nivel_id IN (2, 3) THEN COALESCE(n.nota, 0) END), 0) AS 'NOTA_ED_FISICA',
        COALESCE(SUM(CASE WHEN d.nome = 'LÍNGUA INGLESA' AND d.nivel_id IN (2, 3) THEN COALESCE(n.nota, 0) END), 0) / NULLIF(COUNT(CASE WHEN d.nome = 'LÍNGUA INGLESA' AND d.nivel_id IN (2, 3) THEN COALESCE(n.nota, 0) END), 0) AS 'NOTA_INGLES',
...
def calcular_situacao_final(row, notas_finais, faltas_dict, limite_faltas, disciplinas_map):
    ...
    # As notas da tabela notas_finais já são as finais após recuperação (escala 0-100)
    if all(nota >= 60 for nota in notas_para_verificar.values()):
        ...
        # MAS: Se estamos usando notas_finais (que já incluem recuperação),
        # então o aluno JÁ fez recuperação e essa é a nota final
        if faltas_dict.get(aluno_id, 0) > limite_faltas:
            return 'Reprovada*' if row['SEXO'] == 'F' else 'Reprovado*'
        else:
            return 'Reprovada' if row['SEXO'] == 'F' else 'Reprovado'
...
def criar_tabela_notas(turma_df, notas_finais, faltas_dict, disciplinas_map, ano_letivo_terminado=False):
    # Verificar se a turma é do ensino fundamental I (1º ao 5º ano) ou II (6º ao 9º ano)
    serie = turma_df['NOME_SERIE'].iloc[0]
    serie_numero = int(serie.split('º')[0]) if 'º' in serie else 0
    
    # Definir cabeçalho conforme o nível da série
    if serie_numero >= 1 and serie_numero <= 5:  # Séries iniciais (1º ao 5º ano)
        data = [
            ['Nº', 'NOME DO ALUNO', 
    ...
    else:  # Séries finais (6º ao 9º ano)
        data = [
            ['Nº', 'NOME DO ALUNO', 
            adicionar_quebra_linha("L. PORTUGUESA"), 
            adicionar_quebra_linha("ARTE"), 
    ...
    for row_num, (index, row) in enumerate(turma_df.iterrows(), start=1):
        ...
        if row['STATUS'] == 'Transferido':
            ...
            # Definir as colunas com base no nível da série
            if serie_numero >= 1 and serie_numero <= 5:
                colunas_notas = [
                ...
                colunas_notas = [
                    'NOTA_PORTUGUES', 'NOTA_ARTES', 'NOTA_ED_FISICA',
                    'NOTA_MATEMATICA', 'NOTA_CIENCIAS', 'NOTA_GEOGRAFIA',
                    'NOTA_HISTORIA', 'NOTA_ENS_RELIGIOSO',
    ...
    # Criação da tabela com larguras específicas de acordo com o nível da série
    if serie_numero >= 1 and serie_numero <= 5:  # Séries iniciais
        ...
        for i in range(len(data)):
            if data[i][2] in ['TRANSFERIDO', 'EVADIDO', 'TRANSFERIDA', 'EVADIDA']:
                table.setStyle(TableStyle([
                    ('SPAN', (2, i), (12, i))  # Mescla da coluna 2 até a coluna 12 na linha i
                ]))
