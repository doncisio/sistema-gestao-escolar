...
def verificar_capacidade_turma(turma_id: int) -> Tuple[bool, int, int]:
    ...
    try:
        ...
        capacidade_maxima = turma.get('capacidade_maxima', 0)
        
        tem_vaga = total_alunos < capacidade_maxima
        
        logger.debug(f"Capacidade turma {turma_id}: {total_alunos}/{capacidade_maxima}, vaga={tem_vaga}")
...
def criar_turma(
    ...
    try:
        ...
        if capacidade_maxima <= 0:
            return False, "Capacidade máxima deve ser maior que zero", None
        ...
        for turma in turmas_existentes:
            if turma['nome'].strip().upper() == nome.strip().upper():
        ...
        with get_connection() as conn:
            ...
            cursor.execute("""
                INSERT INTO turmas 
                (nome, turno, capacidade_maxima, ano_letivo_id, serie_id, escola_id, professor_id)
                VALUES (%s, %s, %s, %s, %s, %s, %s)
            """, (nome.strip(), turno, capacidade_maxima, ano_letivo_id, serie_id, escola_id, professor_id))
...
def atualizar_turma(
    ...
    Args:
        ...
        capacidade_maxima: Nova capacidade (None = manter)
        professor_id: Novo professor (None = manter, -1 = remover)
    ...
    try:
        ...
        # Validações
        if turno and turno not in ['Matutino', 'Vespertino', 'Noturno']:
            return False, f"Turno inválido: {turno}"
        
        if capacidade_maxima is not None and capacidade_maxima <= 0:
            return False, "Capacidade máxima deve ser maior que zero"
        
        # Verificar se nova capacidade é menor que total de alunos
        if capacidade_maxima is not None:
            total_alunos = turma.get('total_alunos', 0)
            if capacidade_maxima < total_alunos:
                return False, f"Capacidade não pode ser menor que o total de alunos ({total_alunos})"
        ...
        if capacidade_maxima is not None:
            ...
            valores.append(capacidade_maxima)
        
        if professor_id is not None:
            if professor_id == -1:
                campos.append("professor_id = NULL")
            else:
...
def buscar_turmas(termo: str, ano_letivo_id: Optional[int] = None) -> List[Dict[str, Any]]:
    """
    ...
    try:
        with get_connection() as conn:
            ...
            query += """
                GROUP BY t.id, t.nome, t.turno, t.capacidade_maxima, 
                         t.ano_letivo_id, t.serie_id, s.nome, s.ciclo, al.ano
                ORDER BY s.ciclo, s.nome, t.turno, t.nome
            """
