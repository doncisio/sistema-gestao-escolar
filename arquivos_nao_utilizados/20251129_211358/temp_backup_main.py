...
# Importa+�+�es para o dashboard com gr+�ficos
import matplotlib
matplotlib.use('TkAgg')  # Backend para integra+�+�o com Tkinter
...
def _get_documents_root() -> str:
    """Retorna a pasta raiz onde os documentos ser+�o gerados.
    Procura a vari+�vel de ambiente `DOCUMENTS_SECRETARIA_ROOT` e, se n+�o
    existir, usa por padr+�o o diret+�rio da aplica+�+�o (cwd). Dessa forma a
    estrutura final ser+� `./Documentos Secretaria {ANO}` por padr+�o.
    """
    # 1) Preferir vari+�vel de ambiente quando definida (permite sobrepor sem alterar c+�digo)
    try:
    ...
    try:
        # Import local para evitar poss+�veis efeitos colaterais na importa+�+�o do m+�dulo
    ...
    # 3) +�ltimo recurso: diret+�rio atual da aplica+�+�o (cwd)
...
# -----------------------------
# Helpers para configura+�+�o do Drive
# -----------------------------
...
def _categoria_por_descricao(descricao: str) -> str:
    """Mapeia uma descri+�+�o textual para uma subpasta de documentos."""
    ...
    if 'contato' in d or 'contatos' in d:
        return 'Contatos'
    if 'movimenta+�+�o' in d or 'movimentacao' in d or 'servi+�o' in d or 'servicos' in d:
        return 'Servicos'
    if 'geral' in d:
...
def relatorio_levantamento_necessidades():
    ...
    if _lev and hasattr(_lev, 'gerar_levantamento_necessidades'):
        ...
        messagebox.showerror("Erro", "Fun+�+�o 'gerar_levantamento_necessidades' n+�o dispon+�vel. Verifique o m+�dulo 'levantamento_necessidades'.")
...
def relatorio_contatos_responsaveis():
    ...
    if _cont and hasattr(_cont, 'gerar_pdf_contatos'):
        try:
            # A fun+�+�o `gerar_pdf_contatos` requer o par+�metro `ano_letivo` (ex.: 2025).
        ...
        messagebox.showerror("Erro", "Fun+�+�o 'gerar_pdf_contatos' n+�o dispon+�vel. Verifique o m+�dulo 'Lista_contatos_responsaveis'.")
...
def relatorio_lista_alfabetica():
    ...
    if _alf and hasattr(_alf, 'lista_alfabetica'):
        ...
        messagebox.showerror("Erro", "Fun+�+�o 'lista_alfabetica' n+�o dispon+�vel. Verifique o m+�dulo 'Lista_alunos_alfabetica'.")
...
def relatorio_alunos_transtornos():
    ...
    if _tr and hasattr(_tr, 'lista_alunos_transtornos'):
        ...
        messagebox.showerror("Erro", "Fun+�+�o 'lista_alunos_transtornos' n+�o dispon+�vel. Verifique o m+�dulo 'Lista_alunos_transtornos'.")
...
def relatorio_termo_responsabilidade():
    ...
    if _term and hasattr(_term, 'gerar_termo_responsabilidade'):
        ...
        messagebox.showerror("Erro", "Fun+�+�o 'gerar_termo_responsabilidade' n+�o dispon+�vel. Verifique o m+�dulo 'termo_responsabilidade_empresa'.")
...
def relatorio_tabela_docentes():
    ...
    if _td and hasattr(_td, 'gerar_tabela_docentes'):
        ...
        messagebox.showerror("Erro", "Fun+�+�o 'gerar_tabela_docentes' n+�o dispon+�vel. Verifique o m+�dulo 'tabela_docentes'.")

# Import seguro para a interface de cadastro/edi+�+�o de notas
try:
    import InterfaceCadastroEdicaoNotas as _InterfaceCadastroEdicaoNotas
except Exception:
    _InterfaceCadastroEdicaoNotas = None
# Import opcional de relat+�rios de movimenta+�+�o mensal.
...
# tratamento condicional em tempo de execu+�+�o.
try:
...
# Import seguro para fun+�+�es de notas (NotaAta)
try:
    import NotaAta as _NotaAta
except Exception:
    _NotaAta = None

# Servi+�o de relat+�rios centralizado (m+�dulos legados delegados para services)
try:
...
def lista_reuniao():
    ...
    """
    # Primeira op+�+�o: delegar para o servi+�o centralizado, se dispon+�vel
    ...
    if _gerar_lista_reuniao and hasattr(_gerar_lista_reuniao, 'gerar_lista_reuniao'):
        ...
        messagebox.showerror("Erro", "Fun+�+�o 'gerar_lista_reuniao' n+�o dispon+�vel. Verifique o m+�dulo 'gerar_lista_reuniao'.")
        return None
...
def lista_notas():
    ...
    """
    # Tenta delegar para o servi+�o centralizado quando dispon+�vel
    ...
    if _lista_notas and hasattr(_lista_notas, 'lista_notas'):
        ...
        messagebox.showerror("Erro", "Fun+�+�o 'lista_notas' n+�o dispon+�vel. Verifique o m+�dulo 'Lista_notas'.")
        return None
...
def lista_atualizada_wrapper():
    try:
        if hasattr(Lista_atualizada, 'lista_atualizada'):
            ...
            messagebox.showerror("Erro", "Fun+�+�o 'lista_atualizada' n+�o dispon+�vel no m+�dulo Lista_atualizada.")
    except Exception as e:
...
def lista_atualizada_semed_wrapper():
    try:
        if hasattr(Lista_atualizada_semed, 'lista_atualizada'):
            ...
            messagebox.showerror("Erro", "Fun+�+�o 'lista_atualizada' n+�o dispon+�vel no m+�dulo Lista_atualizada_semed.")
    except Exception as e:
        messagebox.showerror("Erro", f"Falha ao gerar Lista Atualizada SEMED: {e}")
    # NOTE: N+�o usar `else:` aqui ��� bloco `else` em `try/except/else` executa quando
    # n+�o houve exce+�+�o, o que causava a exibi+�+�o indevida de uma mensagem de erro
    # mesmo quando a gera+�+�o foi agendada com sucesso. Removido para comportamento correto.

def gerar_relatorio_notas(*args, **kwargs):
    """Wrapper para `NotaAta.gerar_relatorio_notas`"""
    # Primeira op+�+�o: delegar para o service centralizado quando dispon+�vel
    ...
    if _NotaAta and hasattr(_NotaAta, 'gerar_relatorio_notas'):
        ...
        messagebox.showerror("Erro", "Fun+�+�o 'gerar_relatorio_notas' n+�o dispon+�vel. Verifique o m+�dulo 'NotaAta'.")
        return None
...
def gerar_relatorio_notas_com_assinatura(*args, **kwargs):
    ...
    if _NotaAta and hasattr(_NotaAta, 'gerar_relatorio_notas_com_assinatura'):
        ...
        messagebox.showerror("Erro", "Fun+�+�o 'gerar_relatorio_notas_com_assinatura' n+�o dispon+�vel. Verifique o m+�dulo 'NotaAta'.")
        return None
...
def relatorio_movimentacao_mensal(numero_mes):
    ...
    if report_service is not None and hasattr(report_service, 'gerar_relatorio_movimentacao_mensal'):
        try:
            ...
            messagebox.showerror("Erro", f"Falha ao gerar relat+�rio de movimenta+�+�o: {e}")
            return None
    ...
    messagebox.showerror("Erro", "Fun+�+�o 'relatorio_movimentacao_mensal' n+�o dispon+�vel. Verifique o m+�dulo 'movimentomensal' ou o servi+�o de relat+�rios.")
    return None
...
def boletim(aluno_id, ano_letivo_id=None):
    ...
    messagebox.showerror("Erro", "Fun+�+�o 'boletim' n+�o dispon+�vel. Verifique o m+�dulo 'boletim' ou o servi+�o de relat+�rios.")
    return None
...
def nota_bimestre(bimestre=None, preencher_nulos=False):
    if _NotaAta and hasattr(_NotaAta, 'nota_bimestre'):
        ...
        messagebox.showerror("Erro", "Fun+�+�o 'nota_bimestre' n+�o dispon+�vel. Verifique o m+�dulo 'NotaAta'.")
        return None
...
def nota_bimestre2(bimestre=None, preencher_nulos=False):
    if _NotaAta and hasattr(_NotaAta, 'nota_bimestre2'):
        ...
        messagebox.showerror("Erro", "Fun+�+�o 'nota_bimestre2' n+�o dispon+�vel. Verifique o m+�dulo 'NotaAta'.")
        return None
...
def nota_bimestre_com_assinatura(bimestre=None, preencher_nulos=False):
    if _NotaAta and hasattr(_NotaAta, 'nota_bimestre_com_assinatura'):
        ...
        messagebox.showerror("Erro", "Fun+�+�o 'nota_bimestre_com_assinatura' n+�o dispon+�vel. Verifique o m+�dulo 'NotaAta'.")
        return None
...
def nota_bimestre2_com_assinatura(bimestre=None, preencher_nulos=False):
    if _NotaAta and hasattr(_NotaAta, 'nota_bimestre2_com_assinatura'):
        ...
        messagebox.showerror("Erro", "Fun+�+�o 'nota_bimestre2_com_assinatura' n+�o dispon+�vel. Verifique o m+�dulo 'NotaAta'.")
        return None
...
def abrir_interface_ata(janela_pai=None, status_label=None):
    if _AtaGeral and hasattr(_AtaGeral, 'abrir_interface_ata'):
        ...
        try:
            ...
            messagebox.showerror("Erro", f"Fun+�+�o 'abrir_interface_ata' n+�o dispon+�vel: {e}")
            return None
...
# Defina para False antes de commitar para produ+�+�o.
...
# Import seguro para resumo de ponto ��� importamos o m+�dulo como fallback
try:
...
def gerar_resumo_ponto(*args, **kwargs):
    ...
    messagebox.showerror("Erro", "Fun+�+�o 'gerar_resumo_ponto' n+�o dispon+�vel. Verifique o m+�dulo 'gerar_resumo_ponto' ou o servi+�o de relat+�rios.")
    return None
...
def gerar_folhas_de_ponto(*args, **kwargs):
    ...
    messagebox.showerror("Erro", "Fun+�+�o 'gerar_folhas_de_ponto' n+�o dispon+�vel. Verifique o m+�dulo 'preencher_folha_ponto' ou o servi+�o de relat+�rios.")
    return None
...
# Inicializa o pool de conex+�es no in+�cio da aplica+�+�o para melhor performance
# ============================================================================
...
def criar_dashboard():
    # Delega a cria+�+�o do dashboard ao DashboardManager (instanciado mais abaixo).
    try:
...
def atualizar_dashboard():
    """
    For+�a a atualiza+�+�o do cache e recria o dashboard.
    """
    try:
        if 'dashboard_manager' in globals() and dashboard_manager:
    ...
    # Fallback: limpar cache e recriar via fun+�+�o antiga
...
def criar_tabela():
    ...
    # Nota: Esta fun+�+�o n+�o realiza opera+�+�es diretas ao banco de dados.
    # Dados e exibi+�+�o s+�o constru+�dos a partir de estruturas em mem+�ria (ex.: `df`).
    # N+�o +� necess+�rio abrir/fechar conex+�es aqui; manter o padr+�o de conex+�es curtas em outras fun+�+�es.
    
    # Frame para conter a tabela e sua barra de rolagem
    tabela_frame = Frame(frame_tabela)
    # N+�O fazer pack aqui - ser+� controlado pelo sistema de pesquisa
    ...
    # Cria+�+�o do Treeview com barras de rolagem
    ...
    # Vincular eventos de teclado para navega+�+�o
    ...
    # Adicionar dica/instru+�+�o visual para o usu+�rio
    ...
    # N+�O fazer pack do label - ser+� mostrado junto com a tabela quando necess+�rio
...
def selecionar_item(event):
    ...
    # Adiciona os bot+�es de a+�+�es espec+�ficas para o item selecionado
    ...
    if tipo_item == "Aluno":
        ...
        # ============================================================================
        # OTIMIZA+�+�O 3: Consulta consolidada em uma +�nica query
        ...
        try:
            ...
            # Processar respons+�veis (extra+�+�o segura)
            ...
            if resultado:
                ...
                row_atual = 3  # Come+�ar na linha 3, pois linhas 0, 1 e 2 j+� foram usadas
                ...
                if status == 'Ativo' and data_matricula:
                    ...
                    # Adicionar informa+�+�es de s+�rie e turma para alunos ativos
                    if serie_nome:
                        Label(detalhes_info_frame, 
                    ...
                    if turma_nome and isinstance(turma_nome, str) and turma_nome.strip():
                        ...
                        # Se o nome da turma estiver vazio, mostrar "Turma +�nica" ou o ID
                    ...
                    if serie_nome:
                        Label(detalhes_info_frame, 
                              text=f"+�ltima S+�rie: {serie_nome}", 
                              bg=co1, fg=co0, font=('Ivy 10'), anchor=W).grid(row=1, column=2, sticky=EW, padx=5, pady=3)

                    if turma_nome and isinstance(turma_nome, str) and turma_nome.strip():
                        Label(detalhes_info_frame, 
                              text=f"+�ltima Turma: {turma_nome}", 
                              bg=co1, fg=co0, font=('Ivy 10'), anchor=W).grid(row=3, column=0, sticky=EW, padx=5, pady=3)
                    else:
                        # Se o nome da turma estiver vazio, mostrar "Turma +�nica" ou o ID
                        # J+� temos o turma_id da consulta anterior
                        turma_texto = f"+�ltima Turma: Turma {turma_id}" if turma_id else "+�ltima Turma: N+�o definida"
                        Label(detalhes_info_frame, 
                              text=turma_texto, 
                              bg=co1, fg=co0, font=('Ivy 10'), anchor=W).grid(row=3, column=0, sticky=EW, padx=5, pady=3)
...
def on_select(event):
    # Fun+�+�o para eventos de teclado - aguarda um pouco para a sele+�+�o ser atualizada
    # Usa after() para garantir que a sele+�+�o do treeview seja atualizada primeiro
    def processar_selecao():
        ...
        # Criar bot+�es de a+�+�o primeiro (no topo)
        criar_botoes_frame_detalhes(tipo_item, valores)
        
        # Frame para exibir os detalhes em grid (abaixo dos bot+�es)
        detalhes_info_frame = Frame(frame_detalhes, bg=co1)
        detalhes_info_frame.pack(fill=BOTH, expand=True, padx=10, pady=5)
        ...
        # Exibir informa+�+�es conforme o tipo
        if tipo_item == "Aluno":
            # Linha 1: ID, Nome
            Label(detalhes_info_frame, text=f"ID: {valores[0]}", bg=co1, fg=co0, 
                  font=('Ivy 10 bold'), anchor=W).grid(row=0, column=0, sticky=EW, padx=5, pady=3)
            ...
            # ============================================================================
            # OTIMIZA+�+�O 3: Consulta consolidada (mesmo padr+�o da fun+�+�o selecionar_item)
            # ============================================================================
            cursor = None
            try:
                with get_connection() as conn:
    ...
    # Agendar processamento ap+�s a sele+�+�o ser atualizada
    treeview.after(10, processar_selecao)

def criar_botoes_frame_detalhes(tipo, values):
    # Limpa quaisquer bot+�es existentes antes de criar novos
    for widget in frame_detalhes.winfo_children():
        widget.destroy()

    # Frame para os bot+�es
    acoes_frame = Frame(frame_detalhes, bg=co1)
    acoes_frame.pack(fill=X, padx=10, pady=10)

    # Configurar grid do frame de a+�+�es
    ...
    if tipo == "Aluno":
        ...
        if tem_matricula_ativa or tem_historico:
            ...
            # Se tem matr+�cula ativa, mostrar tamb+�m bot+�o de Declara+�+�o e Editar Matr+�cula
            if tem_matricula_ativa:
                Button(acoes_frame, text="Declara+�+�o", command=lambda: gerar_declaracao(id_item),
        ...
        Button(acoes_frame, text="Declara+�+�o", command=lambda: gerar_declaracao_funcionario(id_item),
...
def verificar_historico_matriculas(aluno_id):
    ...
    WHERE 
        f.cargo IN ('Administrador do Sistemas','Gestor Escolar','Professor@','Auxiliar administrativo',
            'Agente de Portaria','Merendeiro','Auxiliar de servi+�os gerais','T+�cnico em Administra+�+�o Escolar',
...
def matricular_aluno(aluno_id):
    ...
    """
    try:
        # Obter informa+�+�es do aluno e do ano letivo atual usando conex+�es curtas
        with get_connection() as conn:
            cursor = conn.cursor()
        ...
        # Fun+�+�o para carregar turmas com base na s+�rie selecionada
        ...
        # Fun+�+�o ao fechar a janela de matr+�cula
        def ao_fechar_janela():
            janela_matricula.destroy()

        # Configurar a+�+�o de fechamento da janela
...
def excluir_aluno_com_confirmacao(aluno_id):
    ...
    resposta = messagebox.askyesno("Confirma+�+�o", "Tem certeza que deseja excluir este aluno?")
...
def excluir_funcionario_com_confirmacao(funcionario_id):
    # Pergunta ao usu+�rio para confirmar a exclus+�o
    resposta = messagebox.askyesno("Confirma+�+�o", "Tem certeza que deseja excluir este funcion+�rio?")
        ...
        try:
            with get_connection() as conexao:
                ...
                try:
                    ...
                    # Exclui associa+�+�es com funcionario_disciplinas
...
def editar_aluno_e_destruir_frames():
    ...
    try:
        ...
        # Abrir a interface de edi+�+�o em uma nova janela
        janela_edicao = Toplevel(janela)
        from InterfaceEdicaoAluno import InterfaceEdicaoAluno
        
        # Configurar a janela de edi+�+�o antes de criar a interface
        ...
        # Criar a interface de edi+�+�o ap+�s configurar a janela
        app_edicao = InterfaceEdicaoAluno(janela_edicao, aluno_id, janela_principal=janela)
        
        # Atualizar a tabela quando a janela de edi+�+�o for fechada
        def ao_fechar_edicao():
            # Restaurar a janela principal
            janela.deiconify()
            # Atualizar a tabela para refletir as altera+�+�es
            atualizar_tabela_principal()
            # Destruir a janela de edi+�+�o
        ...
        messagebox.showerror("Erro", f"Erro ao abrir interface de edi+�+�o: {str(e)}")
        logger.error(f"Erro ao abrir interface de edi+�+�o: {str(e)}")
...
def gerar_declaracao(id_pessoa=None):
    ...
    # Criar uma janela de di+�logo para selecionar o tipo de declara+�+�o
    dialog = Toplevel(janela)
    dialog.title("Tipo de Declara+�+�o")
    ...
    # Vari+�vel para armazenar a op+�+�o selecionada
    ...
    Label(dialog, text="Selecione o tipo de declara+�+�o:", font=("Ivy", 12), bg=co0, fg=co7).pack(pady=10)
    ...
    # Fun+�+�o para atualizar a visibilidade do campo de motivo
    ...
    # Associar a fun+�+�o ao evento de mudan+�a da op+�+�o
    ...
    def confirmar():
        ...
        # Capturar o motivo se for a op+�+�o "Outros"
        motivo_outros = ""
        ...
        # Executar gera+�+�o da declara+�+�o em background para n+�o bloquear a UI
        def _worker():
        ...
        def _on_done(resultado):
            try:
                if resultado is True or resultado is None:
                    # Alguns geradores retornam None; considerar sucesso se n+�o houve exce+�+�o
                    if status_label is not None:
                        status_label.config(text="Declara+�+�o gerada com sucesso.")
                    messagebox.showinfo("Conclu+�do", "Declara+�+�o gerada com sucesso.")
                else:
                    # Se o worker retornou caminho ou objeto, mostrar informa+�+�o gen+�rica
                    if status_label is not None:
                        status_label.config(text="Declara+�+�o gerada com sucesso.")
                    messagebox.showinfo("Conclu+�do", f"Declara+�+�o gerada: {resultado}")
            except Exception:
                # Evitar que exce+�+�es aqui quebrem a UI
                pass

        def _on_error(exc):
            messagebox.showerror("Erro", f"Falha ao gerar declara+�+�o: {exc}")
            if status_label is not None:
...
def criar_logo():
    ...
    try:
        ...
        app_img = app_img.resize((200, 50))  # Aumentado o tamanho para melhor visualiza+�+�o
...
def criar_pesquisa():
    ...
    # Vincula o evento de pressionar Enter +� fun+�+�o de pesquisa
...
def pesquisar(event=None):
    ...
    try:
        # Remover tudo em frame_tabela e recriar a tabela limpa para evitar sobreposi+�+�o
        for widget in list(frame_tabela.winfo_children()):
            try:
                widget.destroy()
            except Exception:
    ...
    # ============================================================================
    # OTIMIZA+�+�O 5: Pesquisa com FULLTEXT (mais r+�pida que LIKE)
    ...
    try:
        with get_connection() as conn:
            ...
            try:
                ...
                # `Error` n+�o estava importado aqui; capturamos qualquer exce+�+�o
    ...
    if resultados_filtrados:
        ...
        for resultado in resultados_filtrados:
            ...
            # Verifica se h+� campo de data na posi+�+�o 4 e formata
        ...
        # For+�ar atualiza+�+�o visual e foco no primeiro item
        try:
...
# Fun+�+�o para redefinir os frames
def redefinir_frames(titulo):
    # Destruir widgets espec+�ficos nos frames, preservando os bot+�es no frame_dados
    for widget in frame_logo.winfo_children():
        widget.destroy()
...
def criar_acoes():
    # Frame para os bot+�es de a+�+�o
    botoes_frame = Frame(frame_dados, bg=co1)
    botoes_frame.pack(fill=X, expand=True, padx=10, pady=5)
    
    # Configurar grid do frame de bot+�es
    for i in range(7):  # 7 colunas para acomodar todos os bot+�es
        botoes_frame.grid_columnconfigure(i, weight=1)

    # Fun+�+�o para cadastrar novo aluno
    def cadastrar_novo_aluno():
        ...
        # Definir fun+�+�o para atualizar os dados quando a janela de cadastro for fechada
    ...
    # Fun+�+�o para cadastrar novo funcion+�rio
    ...
    # Fun+�+�o para abrir a interface de hist+�rico escolar
    def abrir_historico():
    ...
    try:
        ...
        # Cria uma imagem vazia para evitar erros em bot+�es que usam app_img_matricula
    ...
    # Fun+�+�o para abrir a interface administrativa
    def abrir_interface_administrativa():
        ...
        admin_window.title("Administra+�+�o - Escolas, Disciplinas e Cargas Hor+�rias")
    ...
    try:
        ...
        app_admin = Button(botoes_frame, command=abrir_interface_administrativa, image=app_img_admin,
                          text="Administra+�+�o", compound=LEFT, overrelief=RIDGE, font=('Ivy 11'),
                          bg=co5, fg=co0)
    except FileNotFoundError:
        app_admin = Button(botoes_frame, command=abrir_interface_administrativa, text="Administra+�+�o", 
                          compound=LEFT, overrelief=RIDGE, font=('Ivy 11'),
                          bg=co5, fg=co0)
    ...
    def relatorio():
        ...
        # Mostrar o menu na posi+�+�o do mouse
        try:
            x = janela.winfo_pointerx()
            y = janela.winfo_pointery()
            menu_meses.post(x, y)
        except:
            # Se n+�o conseguir obter a posi+�+�o do mouse, mostrar no centro da janela
    ...
    # Aplicando a fonte +�s op+�+�es do menu
    ...
    # (Movimento Mensal transferido para o menu 'Servi+�os')
    ...
    # Fun+�+�o para abrir a interface de cadastro de notas
    def abrir_cadastro_notas():
        ...
        janela_notas.title("Cadastro/Edi+�+�o de Notas")
        ...
        if _InterfaceCadastroEdicaoNotas:
            ...
            try:
                ...
                messagebox.showerror("Erro", f"N+�o foi poss+�vel abrir a interface de cadastro/edi+�+�o de notas: {e}")
                janela.deiconify()
                return
    
    # Fun+�+�o para abrir o relat+�rio estat+�stico de an+�lise de notas
    ...
    def abrir_solicitacao_professores():
        try:
            ...
            messagebox.showerror("Erro", f"N+�o foi poss+�vel abrir a solicita+�+�o: {e}")

    # Fun+�+�o para abrir o gerenciador de documentos de funcion+�rios
    def abrir_gerenciador_documentos():
        ...
        # Fun+�+�o para quando a janela for fechada
        def ao_fechar():
            janela.deiconify()  # Mostrar a janela principal novamente
            janela_docs.destroy()
    ...
    # Fun+�+�o para abrir o gerenciador de documentos do sistema
    ...
    servicos_menu.add_command(
        label="Solicita+�+�o de Professores e Coordenadores",
    ...
    # Fun+�+�o para abrir a interface de declara+�+�o de comparecimento
    def abrir_interface_declaracao_comparecimento_menu():
        """Abre interface para selecionar aluno e gerar declara+�+�o de comparecimento"""
        ...
        janela_decl.title("Declara+�+�o de Comparecimento de Respons+�vel")
        ...
        # T+�tulo
        Label(frame_principal, text="Gerar Declara+�+�o de Comparecimento", 
        ...
        # Fun+�+�o para carregar respons+�veis quando um aluno for selecionado
        def on_aluno_select(event):
            ...
            aluno_selecionado_label.config(text=f"ԣ� {nome_aluno}", fg=co2)
        ...
        # Vincular evento de sele+�+�o
        listbox_alunos.bind("<<ListboxSelect>>", on_aluno_select)
        
        # Fun+�+�o para gerar
        def gerar():
            # Usar o ID do aluno salvo em vez da sele+�+�o da listbox
            ...
            # Passar os novos par+�metros para a fun+�+�o
        ...
        Button(frame_botoes, text="Gerar Declara+�+�o", command=gerar, 
    ...
    def abrir_interface_crachas():
        ...
        def _worker():
            ...
            caminho = None
            try:
                # Usar o servi+�o centralizado para gerar crach+�s
            ...
            def _on_done():
                ...
                if not success:
                    if isinstance(error_msg, ImportError):
                        messagebox.showerror("Erro de Importa+�+�o", f"N+�o foi poss+�vel importar o m+�dulo de gera+�+�o de crach+�s:\n{error_msg}")
                    else:
    ...
    # Fun+�+�o para abrir importa+�+�o de notas do HTML
    def abrir_importacao_notas_html():
        ...
        try:
            ...
            # Importar e executar o m+�dulo de importa+�+�o
            ...
            messagebox.showerror("Erro", f"Erro ao abrir importa+�+�o de notas: {e}")
            janela.deiconify()
    ...
    # Fun+�+�o para abrir a transi+�+�o de ano letivo
    def abrir_transicao_ano_letivo():
        """Abre interface para transi+�+�o de ano letivo"""
        import os
        ...
        senha_digitada = simpledialog.askstring(
            ...
            "Digite a senha do banco de dados para acessar a Transi+�+�o de Ano Letivo:",
            show='*'
        )
        ...
        if senha_digitada != senha_correta:
            messagebox.showerror(
                "Acesso Negado",
                "Senha incorreta! A transi+�+�o de ano letivo +� uma opera+�+�o cr+�tica\n"
                "e requer autentica+�+�o para prosseguir."
            )
            return
        ...
        try:
            ...
            messagebox.showerror("Erro", f"Erro ao abrir transi+�+�o de ano letivo: {e}")
            traceback.print_exc()
    
    servicos_menu.add_separator()
    servicos_menu.add_command(
        label="���� Transi+�+�o de Ano Letivo",
        command=abrir_transicao_ano_letivo,
        font=menu_font
    )

    menu_bar.add_cascade(label="Servi+�os", menu=servicos_menu)
    ...
    def abrir_dialogo_folhas_ponto():
        ...
        Button(frame, text="Escolher�Ǫ", command=escolher_pasta).grid(row=2, column=2, padx=5)
        ...
        def gerar():
            ...
            try:
                ...
                if status_label is not None:
                    status_label.config(text=f"Gerando folhas de ponto de {nome_mes}/{ano}�Ǫ")

                def _worker():
                    # Executa a gera+�+�o em background e retorna o caminho de sa+�da
                ...
                try:
                    ...
                    messagebox.showerror("Erro", f"Falha ao agendar gera+�+�o das folhas de ponto: {e}")
            except Exception as e:
                if status_label is not None:
                    status_label.config(text="")
    ...
    def abrir_dialogo_resumo_ponto():
        ...
        def gerar():
            ...
            try:
                ...
                if status_label is not None:
                    status_label.config(text=f"Gerando resumo de ponto de {nome_mes}/{ano}�Ǫ")
    ...
    # Fun+�+�o para abrir interface de relat+�rio avan+�ado
    def abrir_relatorio_avancado():
        # Criar janela para configura+�+�o de relat+�rio avan+�ado
        janela_relatorio = Toplevel(janela)
        janela_relatorio.title("Relat+�rio de Notas - Op+�+�es Avan+�adas")
        ...
        # Vari+�veis para armazenar as op+�+�es
        ...
        # Op+�+�es de exibi+�+�o
        Label(frame_principal, text="Op+�+�es de exibi+�+�o:", anchor=W).grid(row=5, column=0, sticky=W, pady=5)
    ...
    # Adicionar as op+�+�es dos bimestres e Ata Geral ao menu
    ...
    # Adicionando separador para as op+�+�es de s+�ries finais
    notas_menu.add_separator()
    ...
    notas_menu.add_separator()
    notas_menu.add_command(label="Relat+�rio Avan+�ado", command=abrir_relatorio_avancado, font=menu_font)
    ...
    relatorios_assinatura_menu.add_separator()
    relatorios_assinatura_menu.add_command(label="Relat+�rio Avan+�ado", command=abrir_relatorio_avancado_com_assinatura, font=menu_font)
    ...
    # Bot+�o de Backup usando o mesmo padr+�o dos outros bot+�es
...
def criar_rodape():
    ...
    # Cria o novo rodap+�
    label_rodape = Label(frame_rodape, text="Criado por Tarcisio Sousa de Almeida, T+�cnico em Administra+�+�o Escolar", 
                         font=('Ivy 10'), bg=co1, fg=co0)
    label_rodape.pack(side=LEFT, padx=10)
    
    # Indicador de backup autom+�tico
    backup_status = Label(frame_rodape, text="���� Backup autom+�tico: ATIVO (14:05 e 17:00 + ao fechar)", 
...
def voltar():
    ...
    # Adicionar uma mensagem de instru+�+�o no frame_detalhes
    instrucao_label = Label(frame_detalhes, text="Selecione um item na tabela para visualizar as op+�+�es dispon+�veis", 
...
# ============================================================================
# OTIMIZA+�+�O 4: Cache de resultados para atualiza+�+�o incremental
# ============================================================================
...
# Cache para dados estat+�sticos do dashboard (atualiza+�+�o a cada 5 minutos)
# ============================================================================
    ...
    try:
        with get_connection() as conn:
            ...
            try:
                ...
                # Estat+�sticas por s+�rie E TURMA - conta ALUNOS +�NICOS e ATIVOS
                cursor.execute("""
                    SELECT 
...
def atualizar_tabela_principal(forcar_atualizacao=False):
    ...
    +�til para refletir altera+�+�es como novos cadastros, edi+�+�es ou exclus+�es.
    
    Args:
        forcar_atualizacao (bool): Se True, ignora o cache e for+�a a atualiza+�+�o
    """
    try:
        ...
        # Se a +�ltima atualiza+�+�o foi h+� menos de 2 segundos, n+�o atualizar
        if not forcar_atualizacao and _cache_dados_tabela['timestamp']:
            ...
            if tempo_decorrido < 2.0:
                logger.debug(f"Cache ainda v+�lido ({tempo_decorrido:.1f}s), pulando atualiza+�+�o")
                return True
        ...
        with get_connection() as conn:
                ...
                if 'query' not in globals() or not globals().get('query'):
                    logger.warning("Vari+�vel 'query' n+�o definida; pulando atualiza+�+�o de tabela")
                    return False
                # Usar a vari+�vel global `query` via globals() e assegurar ao verificador de tipos
                # que se trata de uma string (cast) ��� j+� garantimos acima que 'query' existe e +� truthy.
                ...
                # Calcular hash dos novos dados para verificar mudan+�as
        ...
        # Limpar tabela atual usando try/except para cada opera+�+�o cr+�tica
        try:
            ...
            raise  # Relan+�ar para ser tratado pelo bloco de exce+�+�o principal
...
def editar_funcionario_e_destruir_frames():
    ...
    try:
        ...
        # Criar a interface de edi+�+�o ap+�s configurar a janela
        # A classe InterfaceEdicaoFuncionario j+� gerencia o fechamento e atualiza+�+�o
...
def selecionar_ano_para_boletim(aluno_id):
    ...
    Args:
        aluno_id: ID do aluno
    """
    # Obter informa+�+�es do aluno e anos letivos dentro do contexto de conex+�o
    cursor = None
    try:
        with get_connection() as conn:
            if conn is None:
            ...
            # Preparar janela de sele+�+�o (a interface n+�o depende de manter a conex+�o aberta)
        ...
        for ano_info in anos_letivos:
            ...
            # Armazenar informa+�+�es no dicion+�rio
        ...
        # Criar vari+�vel para armazenar a sele+�+�o
        ...
        # Frame para informa+�+�es (mostrar status da matr+�cula selecionada)
        ...
        # Atualizar informa+�+�es quando o usu+�rio selecionar um ano letivo
        def atualizar_info(*args):
            selected = selected_ano.get()
            if selected in anos_info:
                _, status = anos_info[selected]
                if status == "Transferido":
                    status_label.config(text=f"Observa+�+�o: Aluno transferido no ano letivo selecionado")
                else:
                    status_label.config(text="")
        
        # Vincular fun+�+�o ao evento de sele+�+�o
        selected_ano.trace_add("write", atualizar_info)
        
        # Chamar fun+�+�o uma vez para configura+�+�o inicial
        atualizar_info()
        ...
        # Fun+�+�o para gerar o boletim com o ano letivo selecionado
        def gerar_boletim_selecionado():
            ...
            def _worker():
                if status == 'Transferido':
                    ...
                    # Nota: comunica+�+�o ao usu+�rio ser+� feita no on_done
                    from transferencia import gerar_documento_transferencia
                    gerar_documento_transferencia(aluno_id, ano_letivo_id)
                    return True
                else:
        ...
        messagebox.showerror("Erro", f"Erro ao preparar sele+�+�o de ano letivo: {str(e)}")
        logger.error(f"Erro ao preparar sele+�+�o de ano letivo: {str(e)}")
    finally:
        try:
            if cursor:
                cursor.close()
...
def criar_menu_boletim(parent_frame, aluno_id, tem_matricula_ativa):
    """
    Cria um menu suspenso (Combobox) para sele+�+�o do ano letivo diretamente na interface principal.
    ...
    try:
        ...
        messagebox.showerror("Erro", f"Erro ao obter informa+�+�es de anos letivos: {str(e)}")
        logger.error(f"Erro ao obter informa+�+�es de anos letivos: {str(e)}")
    finally:
        try:
            if cursor:
                cursor.close()
    ...
    # Fun+�+�o para gerar o boletim quando um ano letivo for selecionado
    ...
    # Vincular a fun+�+�o ao evento de sele+�+�o no combobox
...
def editar_matricula(aluno_id):
    ...
    try:
        # Buscar informa+�+�es iniciais em conex+�o curta
        ...
        # Cria a janela de edi+�+�o de matr+�cula
        janela_matricula = Toplevel(janela)
        ...
        # T+�tulo e informa+�+�es iniciais
        Label(frame_matricula, text=f"Edi+�+�o de Matr+�cula", font=("Arial", 14, "bold"), bg=co1, fg=co7).pack(pady=(0, 20))
        info_frame = Frame(frame_matricula, bg=co1)
        info_frame.pack(fill=X, pady=10)
        ...
        # Data da mudan+�a de status
        data_frame = Frame(frame_matricula, bg=co1)
        data_frame.pack(fill=X, pady=10)
        Label(data_frame, text="Data da Mudan+�a de Status (dd/mm/aaaa):", bg=co1, fg=co4).pack(anchor=W, pady=(5, 0))
        ...
        # Fun+�+�o para carregar s+�ries (usa conex+�o curta)
        def carregar_series():
        ...
        # Fun+�+�o para carregar turmas com base na s+�rie selecionada (usa conex+�o curta)
        def carregar_turmas(event=None):
            nonlocal turmas_map
            serie_nome = serie_var.get()
            if not serie_nome:
            ...
            try:
                ...
                for turma in turmas:
                    turma_id, turma_nome, turma_serie_id = turma
                    if not turma_nome or str(turma_nome).strip() == "":
                        turma_nome = f"Turma {turma_id}" if len(turmas) > 1 else "Turma +�nica"
                    turmas_map[turma_nome] = turma_id
        ...
        # Fun+�+�o para salvar a edi+�+�o da matr+�cula (usa conex+�o curta com commit)
        ...
        messagebox.showerror("Erro", f"Erro ao abrir edi+�+�o de matr+�cula: {str(e)}")
        logger.error(f"Erro detalhado: {str(e)}")
...
def selecionar_mes_movimento():
    ...
    # Vari+�vel para armazenar a sele+�+�o
    ...
    combo_mes = ttk.Combobox(frame_mes, values=meses_disponiveis, 
                            ...
                            state="readonly",
                            font=('Ivy', 12))
    combo_mes.current(mes_atual - 1)  # -1 porque o +�ndice come+�a em 0
    combo_mes.pack(pady=10)
    
    def confirmar():
        # Converter o nome do m+�s para seu n+�mero correspondente
        nome_mes = mes_selecionado.get()
        numero_mes = meses.index(nome_mes) + 1  # +1 porque o +�ndice come+�a em 0
        janela_mes.destroy()
...
def abrir_relatorio_avancado_com_assinatura():
    # Criar janela para configura+�+�o de relat+�rio avan+�ado (padronizada com Pend+�ncias)
    janela_relatorio = Toplevel(janela)
    janela_relatorio.title("Relat+�rio de Notas com Assinatura - Op+�+�es Avan+�adas")
    ...
    # Cabe+�alho com destaque (mesmo visual de Pend+�ncias)
    frame_cabecalho = Frame(janela_relatorio, bg=co1, pady=15)
    frame_cabecalho.grid(row=0, column=0, sticky=EW)
    Label(frame_cabecalho, text="���� RELAT+�RIO COM ASSINATURA", font=("Arial", 13, "bold"), bg=co1, fg=co0).pack()
    Label(frame_cabecalho, text="Configura+�+�es avan+�adas para gera+�+�o do relat+�rio de notas com assinatura",
    ...
    # Status de matr+�cula e op+�+�o incluir transferidos
    ...
    # Op+�+�es de exibi+�+�o
    Label(frame_principal, text="Op+�+�es de exibi+�+�o:", anchor=W, bg=co0, font=("Arial", 10, "bold")).grid(row=6, column=0, sticky=W, pady=8, padx=(0, 10))
    ...
    # Observa+�+�o
    Label(frame_principal, text="Observa+�+�o:", anchor=W, font=("Arial", 10, "bold"), bg=co0).grid(row=7, column=0, sticky=W, pady=(12, 0))
    Label(frame_principal, text="Este relat+�rio inclui uma coluna para assinatura dos\nrespons+�veis e +� gerado em modo paisagem.", anchor=W, justify=LEFT, bg=co0).grid(row=7, column=1, sticky=W, pady=(12, 0))

    # Frame para bot+�es (na base) - padronizado com Pend+�ncias
    frame_botoes = Frame(janela_relatorio, bg=co0, padx=25, pady=12)
    frame_botoes.grid(row=2, column=0, sticky=EW)
    # Reservar altura fixa para o rodap+� para evitar sobreposi+�+�o com o conte+�do
    try:
    ...
    # Bot+�es usando ttk (mais consistente com temas do Windows)
    btn_cancelar_rel = ttk.Button(frame_botoes, text="ԣ� Cancelar", command=janela_relatorio.destroy, style="Cancel.TButton")
    ...
    # Adicionar log para confirmar cria+�+�o dos bot+�es (ajuda em debugging)
    try:
        logger.debug("Bot+�es do Relat+�rio Avan+�ado criados: gerar=%s cancelar=%s", btn_gerar_rel, btn_cancelar_rel)
    except Exception:
        try:
            logger.debug("Bot+�es do Relat+�rio Avan+�ado criados (fallback)")
        except Exception:
            pass

    # Hover: ajustar estilos dinamicamente caso o tema n+�o respeite map()
    ...
    # Diagnostic: log estado dos bot+�es ap+�s a janela renderizar
    def _log_button_stats():
        try:
            ...
            try:
                ...
                pass
            logger.debug("Relat+�rio Avan+�ado - stats: %s", vals)
        except Exception as e:
            logger.exception("Erro ao logar stats dos bot+�es: %s", e)
    ...
    def _log_parent_frames():
        try:
            ...
            logger.debug("Relat+�rio Avan+�ado - parent frames: %s", stats)
        except Exception as e:
...
def abrir_relatorio_pendencias():
    ...
    # Frame de cabe+�alho com cor destaque
    frame_cabecalho = Frame(janela_pendencias, bg=co1, pady=15)
    frame_cabecalho.pack(fill=X)
    
    Label(frame_cabecalho, text="���� RELAT+�RIO DE PEND+�NCIAS", 
          font=("Arial", 14, "bold"), bg=co1, fg=co0).pack()
    Label(frame_cabecalho, text="Identifique alunos sem notas e disciplinas n+�o lan+�adas", 
    ...
    Label(frame_info, text="�䦴�� Informa+�+�o", font=("Arial", 9, "bold"), 
          bg=co9, fg=co1).pack(anchor=W, padx=10, pady=(5, 2))
    Label(frame_info, text="��� Alunos sem notas lan+�adas em disciplinas espec+�ficas", 
          font=("Arial", 8), bg=co9, fg=co7, justify=LEFT).pack(anchor=W, padx=10)
    Label(frame_info, text="��� Disciplinas sem nenhum lan+�amento de notas", 
    ...
    # Fun+�+�o para gerar um relat+�rio geral (PDF) com todos os bimestres e turmas
    def gerar_relatorio_geral():
        ...
        A implementa+�+�o reutiliza o servi+�o `services.report_service.gerar_relatorio_pendencias`
        ...
        def _worker_geral():
            try:
                ...
                # Evitar que cada gera+�+�o abra o PDF no visualizador externo
                try:
                ...
                for b in bimestres:
                    for n in niveis:
                        try:
                            # Executar a gera+�+�o dentro da pasta de Pendencias para que os PDFs
                ...
                if not gerados:
                    def _no_files():
                        ...
                        messagebox.showinfo("Sem relat+�rios", "N+�o foram gerados relat+�rios para combinar.")
                ...
                def _on_done():
                    ...
                    try:
                        ...
                        if drive_folder_id is None:
                            # Tentar sem parent (vai para raiz da conta do usu+�rio)
                        ...
                        if fid:
                            # Se obtivermos um link webView, mostr+�-lo; sen+�o apenas informar ID
                            ...
                            logger.warning("Upload para Drive n+�o ocorreu: retorno None")
                            try:
                                messagebox.showwarning('Upload para Drive', 'Falha ao enviar o arquivo para o Google Drive. Verifique logs e autoriza+�+�o.')
                            except Exception:
                                pass
                    except Exception as e:
    ...
    # Bot+�es estilizados
    btn_gerar = Button(frame_botoes, text="���� Gerar Relat+�rio", command=gerar_relatorio, 
                      width=17, height=1, bg=co5, fg=co0, font=("Arial", 10, "bold"),
                      relief=RAISED, bd=2, cursor="hand2")
    btn_gerar.pack(side=RIGHT, padx=5)

    # Bot+�o para gerar relat+�rio geral (todos os bimestres)
    btn_relatorio_geral = Button(frame_botoes, text="���� Relat+�rio Geral", command=gerar_relatorio_geral,
                                 width=16, height=1, bg=co4, fg=co0, font=("Arial", 10, "bold"),
                                 relief=RAISED, bd=2, cursor="hand2")
    btn_relatorio_geral.pack(side=RIGHT, padx=5)
    
    btn_cancelar = Button(frame_botoes, text="ԣ� Cancelar", command=janela_pendencias.destroy, 
                         width=12, height=1, bg=co7, fg=co0, font=("Arial", 10, "bold"),
                         relief=RAISED, bd=2, cursor="hand2")
    btn_cancelar.pack(side=RIGHT, padx=5)
    
    # Efeitos hover nos bot+�es
...
# Fun+�+�o para fechar o programa com backup final
def ao_fechar_programa():
    """
    Fun+�+�o chamada quando o usu+�rio fecha a janela principal.
...
criar_frames()
criar_logo()
criar_acoes()  # Isso cria os bot+�es principais
criar_pesquisa()
criar_tabela()
